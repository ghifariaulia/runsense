...
**Always run the app with Docker.** Prefer `docker-compose` commands for local development, verification, and debugging so frontend/backend ports, env loading, and service wiring match the project runtime. Use local `uvicorn` or `npm run dev` only when explicitly requested or when isolating a Docker-specific failure.
