...
async def get_recent_activities(access_token: str, weeks: int = 8) -> list[dict]:
    """Fetch running activities from the last N weeks."""
    since = int((datetime.now() - timedelta(weeks=weeks)).timestamp())
    async with httpx.AsyncClient() as client:
        r = await client.get(
            f"{STRAVA_BASE}/athlete/activities",
            headers={"Authorization": f"Bearer {access_token}"},
            params={"after": since, "per_page": 100},
        )
        r.raise_for_status()
        activities = r.json()

    # Filter runs only, return clean subset
    runs = [
        {
            "id": a["id"],
            "name": a["name"],
